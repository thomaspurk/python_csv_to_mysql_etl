...
def run_column_transformations( df: DataFrame, column_name: str, transformations: list) -> None:
    """ Execute Columns Transformation Functions.
...
def to_unique(df: DataFrame) -> None:
    """ Table transformation to remove duplicate records.
    """
    df.drop_duplicates(inplace=True)

def to_int(df: DataFrame, column_name: str, *args) -> None:
...
def to_text(df: DataFrame, column_name: str, *args) -> None:
    """ Column transformation to convert data into a string
    """
    df[column_name] = df[column_name].astype(str)

def to_date(df: DataFrame, column_name: str, *args) -> None:
...
def to_float(df: DataFrame, column_name: str, *args) -> None:
    """ Column transformation to convert data into a floating point number. 
    TODO - Need to error trap for when data cannot be converted to a decimal number.
    """
    df[column_name] = df[column_name].astype(float)

def append_value(df: DataFrame, column_name: str, *args) -> None:
...
def drop_column(df: DataFrame, column_name: str, *args) -> None:
